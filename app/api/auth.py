...
async def register_user(
    ...
    try:
        ...
        existing_username = AuthService.get_user_by_username(db, username=user.username)
    ...
    except AuthException.USER_ALREADY_EXISTS:
        raise
    except AuthException.USERNAME_TAKEN:
        raise
    except Exception as e:
        logger.error(f"Ошибка при регистрации пользователя: {str(e)}")
        db.rollback()  # ДОБАВЛЕНО: откат транзакции
        raise HTTPException(
...
async def login_user(
    ...
    except AuthException.INVALID_CREDENTIALS:
        raise
    except AuthException.INACTIVE_USER:
        raise
    except Exception as e:
...
async def get_token_info(request: Request):
    ...
    try:
        ...
        return {
            "user_id": str(user_id),  # ИСПРАВЛЕНО: конвертируем UUID в строку
            "email": email,
            "authenticated": True
        }
...
async def verify_token_from_header(token: str):
    ...
    try:
        ...
        return {"user_data": {
            "user_id": str(user_data['user_id']),  # ИСПРАВЛЕНО: UUID в строку
